...
class SimpleWorkingAIController:
    ...
    def _get_sumo_path(self) -> str:
        ...
        try:
            result = subprocess.run(["where", "sumo-gui"], capture_output=True, text=True, encoding='utf-8')
    ...
    def start_simulation(self, gui: bool = True, auto_start: bool = True) -> bool:
        ...
        try:
            ...
            if os.path.exists(self.sumo_path):
                if auto_start:
                    ...
                    sumo_cmd = [self.sumo_path, "-c", self.sumo_config]
            else:
                if auto_start:
            ...
            traci.start(sumo_cmd, port=8813, numRetries=3)
            self.is_running = True
            
            if auto_start:
                # Initialize traffic lights only if auto-starting
                ...
                print("SUMO GUI opened - Click 'Run' to start simulation")
            
            return True
            
        except Exception as e:
            error_msg = str(e).encode('ascii', 'ignore').decode('ascii')
            print(f"Failed to start SUMO: {error_msg}")
            self.is_running = False
            return False
    
    def start_ai_control(self):
        """Start AI control loop (call this when user clicks Run in SUMO)"""
        try:
            ...
            print(f"AI Control activated for {self.junction_ids}")
            print("AI traffic control is now running")
            return True
